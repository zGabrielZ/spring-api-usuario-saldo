...
class SaldoServiceTest {
    ...
    void deveMostrarSaldosPorUsuario(){
        ...
        // Executando
        List<Saldo> saldosResultados = saldoService.saldosPorUsuario(idUsuarioPesquisar);

        // Verificando
        assertThat(saldosResultados).isNotEmpty();
        assertThat(saldosResultados.get(0).getDeposito()).isEqualTo(BigDecimal.valueOf(200.00));
        assertThat(saldosResultados.get(1).getDeposito()).isEqualTo(BigDecimal.valueOf(500.00));
        assertThat(saldosResultados.get(2).getDeposito()).isEqualTo(BigDecimal.valueOf(600.00));
        assertThat(saldosResultados.get(3).getDeposito()).isEqualTo(BigDecimal.valueOf(800.00));

    }

    @Test
    @DisplayName("Saldos por usuário não deveria retornar uma lista quando não tiver saldos associado ao usuário pesquisado.")
    void naoDeveMostrarSaldosPorUsuarioNaoEncontrado(){
        // Cenário
        Long idUsuarioPesquisar = 1L;
        ...
        // Executando e verificando
        assertThrows(RecursoNaoEncontrado.class, () -> saldoService.saldosPorUsuario(idUsuarioPesquisar));
    }
